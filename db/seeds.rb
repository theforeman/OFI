...
used_in_ui = Staypuft::Service::UI_PARAMS.inject([]) { |arr, (service, params)| arr + params }.uniq

roles.each do |role_data|
  #create role
  role = Staypuft::Role.where(:name => role_data[:name]).first_or_create!

  # set params in puppetclass
  if role_data[:class]
    puppet_class = Puppetclass.find_by_name role_data[:class]
    # skip if puppet class isn't found (yet)
    if puppet_class
      puppet_class.class_params.each do |class_param|
        case
