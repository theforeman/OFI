...
$(document).on('change', 'fieldset#interface', function(event) { update_fencing(event); });

function update_fencing(event) {
  function get_value_from_network_interface(fieldset, field, html_input_type) {
  ...
  interface_fieldset = $(event.target).parentsUntil('fieldset#interface').parent();
  selected_provider = get_value_from_network_interface(interface_fieldset, 'provider', 'select');
  if (selected_provider.length == 0 || selected_provider.val() != 'IPMI') {
    return;
  }

  ip = get_value_from_network_interface(interface_fieldset, 'ip', 'input').val();
  $('#host_fencing_attrs_fence_ipmilan_address').val(ip);
  username = get_value_from_network_interface(interface_fieldset, 'username', 'input').val();
  $('#host_fencing_attrs_fence_ipmilan_username').val(username);
  password = get_value_from_network_interface(interface_fieldset, 'password', 'input').val();
...
function submit_host(){
  ...
  if(/\/clone$/.test(window.location.pathname)){ url = foreman_url('/staypuft_hosts'); }
